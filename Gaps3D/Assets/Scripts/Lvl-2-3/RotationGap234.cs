...
public class RotationGap234 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(2.4f, gameObject.transform.position.y, -0.4f);
			...
			{
				transform.position = new Vector3(0.25f, gameObject.transform.position.y, -2.9f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(2.75f, -2, -2.4f);
