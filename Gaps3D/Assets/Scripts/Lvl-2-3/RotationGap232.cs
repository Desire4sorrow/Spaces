...
public class RotationGap232 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(-2.8f, gameObject.transform.position.y, -0.3f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(-2.4f, gameObject.transform.position.y, -2.8f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(0, gameObject.transform.position.y, -2.5f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(-0.2f, -2, -0.4f);
