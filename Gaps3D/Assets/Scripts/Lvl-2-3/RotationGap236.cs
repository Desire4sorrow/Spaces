...
public class RotationGap236 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(-5.4f, gameObject.transform.position.y, -2.6f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(-3.3f, gameObject.transform.position.y, -2.4f);
			...
			{
				transform.position = new Vector3(-5.55f, -2, -0.45f);
