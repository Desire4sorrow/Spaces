...
public class RotationGap225 : MonoBehaviour
{
	...
	void OnMouseDown()
    {
		...
		if (!_isObjectMoved)
		{
			_resetPosition = GameObject.Find("Gap5").transform.position;
			_isObjectMoved = true; 
		}
    }
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(-3.3f, gameObject.transform.position.y, -2.85f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(gameObject.transform.position.x, gameObject.transform.position.y, -0.65f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(-5.3f, gameObject.transform.position.y, gameObject.transform.position.z);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(-5.5f, 0, -2.45f);
