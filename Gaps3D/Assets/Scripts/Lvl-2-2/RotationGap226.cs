...
public class RotationGap226 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(2.7f, gameObject.transform.position.y, -2.8f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(2.5f, gameObject.transform.position.y, -0.6f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(0.05f, gameObject.transform.position.y, gameObject.transform.position.z);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(0.5f, -2, -3);
