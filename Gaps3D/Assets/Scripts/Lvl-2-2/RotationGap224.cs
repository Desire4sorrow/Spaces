...
public class RotationGap224 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(8.5f, gameObject.transform.position.y, -0.45f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(6.5f, -2, gameObject.transform.position.z);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(7, -2.1f, -2.05f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(8.6f, -2, -2.45f);
