...
public class RotationGap214 : MonoBehaviour
{
	...
	void OnMouseDown()
    {
		...
		if (!_isObjectMoved)
		{
			_resetPosition = GameObject.Find("Gap4").transform.position;
			_isObjectMoved = true; 
		}
    }
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(-2.6f, gameObject.transform.position.y, gameObject.transform.position.z);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(-2.4f, gameObject.transform.position.y, -2.6f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(-0.1f, gameObject.transform.position.y, -2.4f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(-0.3f, -2, -0.3f);
