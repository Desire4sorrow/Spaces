...
public class RotationGap215 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(3.1f, gameObject.transform.position.y, -2.8f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(5.6f, gameObject.transform.position.y, -2.25f);
				_position = Position.Back;
			}
			else if (_position == Position.Back)
			{
				transform.position = new Vector3(5.2f, gameObject.transform.position.y, -0.4f);
				_position = Position.Left;
			}
			else if (_position == Position.Left)
			{
				transform.position = new Vector3(2.85f, -2, -0.3f);
