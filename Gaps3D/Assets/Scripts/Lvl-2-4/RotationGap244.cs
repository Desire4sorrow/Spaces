...
public class RotationGap244 : MonoBehaviour
{
	...
	void OnMouseUp()
    {
		...
		if (AreObjectsInSamePosition(_resetPosition, gameObject.transform.position))
		{
			...
			{
				transform.position = new Vector3(-2.5f, -2, -2.5f);
				_position = Position.Right;
			}
			else if (_position == Position.Right)
			{
				transform.position = new Vector3(-0.4f, gameObject.transform.position.y, gameObject.transform.position.z);
			...
			{
				transform.position = new Vector3(-2.5f, -2, -0.3f);
